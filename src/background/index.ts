...
declare global {
  ...
  var getTimeWiseFocusDistractionCounters: (() => Promise<unknown>) | undefined;
}

function ensureFocusDistractionHeartbeat(): void {
  chrome.alarms.create(FOCUS_DISTRACTION_HEARTBEAT_ALARM, { periodInMinutes: 1 });
}
...
chrome.windows.onFocusChanged.addListener(async (windowId) => {
  ...
  runtimeContext.runtimeState.currentWindowId = isWindowFocused ? windowId : null;
  if (isWindowFocused) {
    await refreshActiveTab(runtimeContext);
  }
});
...
globalThis.getTimeWiseFocusDistractionCounters = () => getDebugFocusDistractionCounters(runtimeContext);
