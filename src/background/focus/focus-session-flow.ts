    ...
    devDebugLog("focusNudge.decision", {
        ...
        source:
            decision.action === "block"
                ? decision.matchedRule?.source || null
                : null,
    });
        ...
        primaryAction:
            activeSession?.status === "active"
                ? {
                      ...
                      label: t("popup.buttonStop"),
                  }
                : {
                      type: MESSAGE_TYPES.startFocusSession,
                      label: t("popup.buttonStart"),
                  },
        secondaryActions: [],
...
export async function forceFocusNudge(
    context: BackgroundRuntimeContext,
): Promise<{ ok: boolean; response: unknown }> {
    const settings = await getSettings();
    const t = createTranslator(settings.language);
    const host = context.runtimeState.currentHost || t("nudge.currentSite");
    return showFocusNudge(context, t("nudge.message"), {
        sessionId: "manual",
        host,
        category: "other",
        presentation: "strict",
