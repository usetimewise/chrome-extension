...
function isBrowserActivityTrackable(): boolean {
  return browserActivityState.isWindowFocused && browserActivityState.idleState === "active";
}

export function setFocusDistractionWindowFocused(isWindowFocused: boolean): void {
  browserActivityState.isWindowFocused = isWindowFocused;
}

export function setFocusDistractionIdleState(idleState: chrome.idle.IdleState): void {
  browserActivityState.idleState = idleState;
}

export async function syncFocusDistractionCounterSession(now = Date.now()): Promise<FocusDistractionCountersState> {
  const [sessions, state] = await Promise.all([getFocusSessions(), getFocusDistractionCounters()]);
  const nextState = ensureFocusDistractionCounterSession(state, activeFocusSession(sessions), now);
  if (nextState !== state) {
...
export async function flushFocusDistractionTracking(
  ...
  const sessionState = ensureFocusDistractionCounterSession(storedState, activeSession, now);
  ...
  if (!activeSession || !startedAt || !isBrowserActivityTrackable() || !isTrackableUrl(context.runtimeState.currentUrl)) {
    return sessionState === storedState ? storedState : saveFocusDistractionCounters(sessionState);
  }
  ...
  const decision = await decideDistractionSite(context.runtimeState.currentUrl, {
    siteRules: {
  ...
  return saveFocusDistractionCounters(addFocusDistractionDuration(sessionState, {
