...
function IconPreviewApp() {
  ...
  return (
    <main className="icon-preview-shell">
      ...
      <section className="icon-preview-grid" aria-label="Production icon set">
        {iconNames.map((name) => (
          <article className="icon-preview-card" key={name}>
            <div className="icon-preview-samples">
              {ICON_SIZES.map((size) => (
                <span className="icon-preview-sample" key={size}>
                  <AppIcon name={name} size={size} className={name === "loading" ? "is-spinning" : undefined} />
                </span>
              ))}
            </div>
            <div>
      ...
      <section className="icon-preview-contexts" aria-label="Icon contexts">
        <button className="settings-icon-button" type="button" aria-label="Settings">
          <AppIcon name="settings" />
        </button>
        <button className="blocked-host-add" type="button" aria-label="Add host">
          <AppIcon name="add" />
        </button>
        <button className="blocked-host-action" type="button" aria-label="Remove host">
