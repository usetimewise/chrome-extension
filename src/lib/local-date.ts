...
function formatterFor(timezone: string, granularity: DatePartGranularity): Intl.DateTimeFormat {
...
export function localDateParts(
  ...
  const parts = formatterFor(normalizedTimezone(timezone), granularity).formatToParts(date);
...
export function utcInstantForLocalTime(dateKey: string, hour: number, timezone?: string | null): string {
