...
export type BackgroundRequestOf<TType extends BackgroundRequestType> = Extract<
    BackgroundRequest,
    { type: TType }
>;
...
export function isBackgroundErrorResponse(
    value: unknown,
): value is BackgroundErrorResponse {
    return (
        isPlainObject(value) &&
        value.ok === false &&
...
export type BackgroundSuccessResponse<TType extends BackgroundRequestType> =
    BackgroundSuccessResponseMap[TType];
...
export type ContentRequestOf<TType extends ContentRequestType> = Extract<
    ContentRequest,
    { type: TType }
>;
...
export type ContentResponse<TType extends ContentRequestType> =
    ContentResponseMap[TType];
...
function isUserPreferences(value: unknown): value is UserPreferences {
    return (
        ...
        (value.disabledDefaultBlockRuleIds === undefined ||
            isStringArray(value.disabledDefaultBlockRuleIds)) &&
...
export function isBackgroundRequest(
    ...
    switch (value.type) {
        ...
        case MESSAGE_TYPES.dismissFocusOffer:
            return (
                (value.action === "defer" || value.action === "close") &&
                isRequiredString(value.host)
            );
        default:
            return false;
