...
export async function sendBackgroundMessage<TType extends BackgroundRequestType>(
...
export async function sendContentMessage<TType extends ContentRequestType>(
  ...
  return chrome.tabs.sendMessage(tabId, message) as Promise<ContentResponse<TType>>;
}
