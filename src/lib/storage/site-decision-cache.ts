...
function splitURLPrefixPattern(
    pattern: string,
): { host: string; pathname: string } | null {
...
function entryMatchesURL(
    entry: SiteDecisionCacheEntry,
    rawUrl: string,
    focusMode: SiteDecisionFocusMode,
): boolean {
    ...
    if (entry.cacheType === "url_prefix") {
        ...
        return Boolean(
            prefix &&
            parts.host === prefix.host &&
            isPathPrefix(parts.pathname, prefix.pathname),
        );
    }
...
export async function getSiteDecisionCache(): Promise<SiteDecisionCacheState> {
    const cache = await getFromStorage<Partial<SiteDecisionCacheState> | null>(
        STORAGE_KEYS.siteDecisionCache,
        null,
    );
    return {
...
export async function saveSiteDecisionCache(
    cache: SiteDecisionCacheState,
    now = Date.now(),
): Promise<SiteDecisionCacheState> {
...
export async function findCachedSiteDecision(
    ...
    return (
        entries.find((entry) => entryMatchesURL(entry, rawUrl, focusMode)) ||
        null
    );
}
