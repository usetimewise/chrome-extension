...
function normalizeFocusOfferPromptEvents(
    value: unknown,
    ...
    return value
        .filter(
            (event): event is FocusOfferPromptEvent =>
                Boolean(event) &&
                typeof event === "object" &&
                ["shown", "deferred", "closed", "started"].includes(
                    (event as FocusOfferPromptEvent).type,
                ) &&
...
export async function getRuntimeState(): Promise<RuntimeState> {
    const runtime = await getFromStorage<Partial<RuntimeState> | null>(
