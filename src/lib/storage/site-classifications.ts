...
import type {
    Category,
    SiteClassificationRecord,
    SiteClassificationState,
    SiteClassificationStatus,
} from "../types.js";
...
export function effectiveCategoryForHost(
    ...
    if (
        (options.excludedHosts || []).some((rule) =>
    ...
    for (const [rule, category] of Object.entries(
...
export async function getSiteClassifications(): Promise<SiteClassificationState> {
    const state = await getFromStorage<SiteClassificationState | null>(
        STORAGE_KEYS.siteClassifications,
        null,
    );
    return normalizeState(state);
}

export async function saveSiteClassifications(
    state: SiteClassificationState,
...
export async function getSiteClassification(
    host: string | null | undefined,
): Promise<SiteClassificationRecord | null> {
    const normalizedHost = normalizeHost(host);
    if (!normalizedHost) {
        return null;
    }
...
export async function ensureSiteClassificationPending(
    host: string,
): Promise<SiteClassificationState> {
    const normalizedHost = normalizeHost(host);
    if (!normalizedHost) {
...
export async function scheduleSiteClassificationRetry(
    ...
    const status: SiteClassificationStatus =
        attempts >= MAX_CLASSIFICATION_ATTEMPTS ? "failed" : "retry_scheduled";
    const nextRetryAt =
        status === "failed"
            ? null
            : new Date(
                  Date.now() + classificationBackoffDelayMs(attempts),
              ).toISOString();
...
export async function clearSiteClassification(
    host: string,
): Promise<SiteClassificationState> {
    const normalizedHost = normalizeHost(host);
    const state = await getSiteClassifications();
...
export async function getHostsReadyForClassification(
    limit = 100,
    now = Date.now(),
): Promise<string[]> {
    const state = await getSiteClassifications();
    return Object.entries(state.byHost)
        ...
        .sort(([, left], [, right]) => {
            ...
            const rightAt = right.nextRetryAt
                ? Date.parse(right.nextRetryAt)
                : 0;
...
export function selectHostsForForcedClassification(
    state: SiteClassificationState,
    limit = 100,
): string[] {
    return Object.entries(state.byHost)
...
export async function getNextClassificationRetryAt(): Promise<number | null> {
    ...
    for (const record of Object.values(state.byHost)) {
        ...
        nextRetryAt =
            nextRetryAt === null ? retryAt : Math.min(nextRetryAt, retryAt);
    }
