...
function latestCompletedSessionEndedAt(
...
export function getFocusOfferEligibility(
    ...
    if (
        latestEndedAt !== null &&
        now - latestEndedAt < FOCUS_OFFER_SESSION_COOLDOWN_MS
    ) {
    ...
    if (
        latestDeferred &&
        now - latestDeferred.at < FOCUS_OFFER_DEFER_COOLDOWN_MS
    ) {
