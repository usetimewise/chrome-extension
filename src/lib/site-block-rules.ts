...
import {
    lookupCachedUrlDecision,
    lookupUrlDecision,
} from "./urlDecision/match.js";
...
const DEFAULT_SITE_BLOCK_RULE_IDS = new Set(
    DEFAULT_SITE_BLOCK_RULES.map((rule) => rule.id),
);

export function normalizeDisabledDefaultBlockRuleIds(
    value: readonly string[] | null | undefined,
): string[] {
    return Array.from(
        new Set(
            (value || []).filter((id) => DEFAULT_SITE_BLOCK_RULE_IDS.has(id)),
        ),
    ).sort();
}

function buildUserBlockRules(
    siteRules: SiteRuleState | null | undefined,
): SiteBlockRule[] {
    return Object.entries(siteRules?.categoryOverrides || {})
        .filter(([, category]) =>
            DISTRACTION_CATEGORIES.has(category as Category),
        )
...
export function buildEnabledDefaultBlockRules(
    disabledRuleIds: readonly string[] = [],
): SiteBlockRule[] {
    const disabledIds = new Set(
        normalizeDisabledDefaultBlockRuleIds(disabledRuleIds),
    );
...
function splitPattern(
    pattern: string,
): { host: string; pathSegments: string[] } | null {
    const normalized = normalizeUrl(`https://${pattern}`);
    return normalized
        ? { host: normalized.domain, pathSegments: normalized.pathSegments }
        : null;
}

function isUserAllowedHost(
    host: string,
    siteRules: SiteRuleState | null | undefined,
): boolean {
    if (!siteRules) {
    ...
    if (
        (siteRules.excludedHosts || []).some((rule) =>
    ...
    return Object.entries(siteRules.categoryOverrides || {}).some(
        ([ruleHost, category]) =>
            hostMatchesRule(host, ruleHost) &&
            FOCUS_CATEGORIES.has(category as Category),
    );
}

function findUserBlockedRule(
    host: string,
    siteRules: SiteRuleState | null | undefined,
): SiteBlockRule | null {
    return (
        buildUserBlockRules(siteRules).find((rule) =>
            hostMatchesRule(host, rule.pattern),
        ) || null
    );
}
...
function matchRule(rawUrl: string, rule: SiteBlockRule): boolean {
    ...
    if (
        !parsedRule ||
        !parentDomainCandidates(normalized.domain).includes(parsedRule.host)
    ) {
        return false;
    }
    ...
    return parsedRule.pathSegments.every(
        (segment, index) => normalized.pathSegments[index] === segment,
    );
}

function findDefaultBlockedRule(
    rawUrl: string,
    disabledRuleIds: readonly string[] = [],
): SiteBlockRule | null {
    return (
        buildEnabledDefaultBlockRules(disabledRuleIds).find((rule) =>
            matchRule(rawUrl, rule),
        ) || null
    );
}
...
export function buildLocalBlockRules(
    context: LocalSiteBlockRuleContext = {},
): SiteBlockRule[] {
...
export function isLocalSiteBlocked(
    rawUrl: string,
    context: LocalSiteBlockRuleContext = {},
): boolean {
...
export async function decideDistractionSite(
    ...
    const defaultRule = findDefaultBlockedRule(
        rawUrl,
        context.disabledDefaultBlockRuleIds,
    );
    if (defaultRule) {
    ...
    const lookupDecision = context.allowNetworkLookup
        ? await lookupUrlDecision(rawUrl, {
    ...
    if (lookupDecision.action === "block") {
        return {
            ...
            reason: context.allowNetworkLookup
                ? "lookup_decision"
                : "cached_decision",
            matchedRule: lookupDecision.matchedKey
                ? {
                      ...
                      patternType: lookupDecision.matchedKey.startsWith("d:")
                          ? "domain"
                          : "url_prefix",
                      category: lookupDecision.category || "other",
                      source: "cache",
