...
export function activeFocusSession(sessions: FocusSession[] = []): FocusSession | null {
...
export function shouldResetFocusDistractionCounters(
  ...
  if (!activeSession) {
    return state.sessionId !== null || Object.keys(state.counters).length > 0;
  }
  ...
  return typeof state.lastDistractedAt === "number" &&
...
export function ensureFocusDistractionCounterSession(
  ...
  if (shouldResetFocusDistractionCounters(state, activeSession, now)) {
    return createEmptyFocusDistractionCounters(activeSession?.id ?? null, now);
  }
