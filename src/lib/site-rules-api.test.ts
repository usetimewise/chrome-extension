...
test("classifySites posts domains and returns partial failures", async () => {
  ...
  const response = await classifySites("https://api.example.com", "device-1", ["github.com", "broken.example"]);
  ...
  assert.equal(calls[0]?.init?.body, JSON.stringify({ domains: ["github.com", "broken.example"] }));
});
