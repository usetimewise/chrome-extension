...
test("domain rule matches subdomains", async () => {
    ...
    const cached = await findCachedSiteDecision(
...
test("url prefix rule matches nested paths on the same host", async () => {
    ...
    const nested = await findCachedSiteDecision(
        "https://youtube.com/shorts/abc/comments",
        "normal",
        now + 1_000,
    );
    const otherHost = await findCachedSiteDecision(
...
test("decision without matched rule is cached only for normalized current url", async () => {
    ...
    const samePath = await findCachedSiteDecision(
        "https://example.com/path?x=2",
        "normal",
        now + 1_000,
    );
    const childPath = await findCachedSiteDecision(
...
test("cache pruning keeps the newest entries within the max size", async () => {
    ...
    for (
        let index = 0;
        index < SITE_DECISION_CACHE_MAX_ENTRIES + 5;
        index += 1
    ) {
        await cacheSiteDecision(
    ...
    const cache = storage[STORAGE_KEYS.siteDecisionCache] as {
        entries: Array<{ cacheKey: string }>;
    };

    assert.equal(cache.entries.length, SITE_DECISION_CACHE_MAX_ENTRIES);
    assert.equal(
        cache.entries.some((entry) => entry.cacheKey === "example.com/0"),
        false,
    );
    assert.equal(
        cache.entries.some(
            (entry) =>
                entry.cacheKey ===
                `example.com/${SITE_DECISION_CACHE_MAX_ENTRIES + 4}`,
        ),
        true,
    );
});
