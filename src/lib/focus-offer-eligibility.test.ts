...
function event(
    type: FocusOfferPromptEvent["type"],
    at: number,
): FocusOfferPromptEvent {
    return { type, at, host: "reddit.com" };
}

test("focus offer is eligible immediately without previous sessions", () => {
    assert.deepEqual(getFocusOfferEligibility([], [], 1_000), {
        eligible: true,
    });
});
...
test("focus offer waits eight hours after the latest completed session", () => {
    ...
    assert.deepEqual(getFocusOfferEligibility([recent], [], now), {
        eligible: false,
        reason: "recent_completed_session",
    });
    assert.deepEqual(getFocusOfferEligibility([old], [], now), {
        eligible: true,
    });
});
...
test("focus offer respects defer and close cooldown facts", () => {
    ...
    assert.deepEqual(
        getFocusOfferEligibility(
            [],
            [event("deferred", now - FOCUS_OFFER_DEFER_COOLDOWN_MS + 1)],
            now,
        ),
    ...
    assert.deepEqual(
        getFocusOfferEligibility(
            [],
            [event("closed", now - FOCUS_OFFER_CLOSE_COOLDOWN_MS + 1)],
            now,
        ),
