...
export function buildFocusSessionsView(
    sessions: FocusSession[] = [],
    now = new Date(),
): FocusSessionsView {
    ...
    const activeSession =
        sorted.find((session) => session.status === "active") || null;
    const completed = sorted.filter(
        (session) => session.status === "completed",
    );
    const totalDuration = completed.reduce(
        (sum, session) => sum + Number(session.active_duration_ms || 0),
        0,
    );
    const longest = completed.reduce(
        (max, session) =>
            Math.max(max, Number(session.active_duration_ms || 0)),
        0,
    );
    ...
    return {
        summary: {
            ...
            average_duration_ms: completed.length
