...
export async function fetchLookupBuckets(
    baseUrl: string,
    prefixes: string[],
): Promise<LookupBucketsResponse> {
    ...
    const response = await requestJSON<LookupBucketsResponse>(
        baseUrl,
        "/v1/url-decision/lookup-buckets",
        {
            method: "POST",
    ...
    devDebugLog("urlDecision.lookupBuckets.response", {
        ...
        entryCount: normalizedResponse.buckets.reduce(
...
function normalizeLookupBucketsResponse(
    response: LookupBucketsResponse,
): LookupBucketsResponse {
    return {
        ...response,
