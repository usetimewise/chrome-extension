...
export async function lookupUrlDecision(
    rawUrl: string,
    userContext: UserContext = {},
): Promise<UrlDecisionResult> {
    ...
    try {
        ...
        const prefixes = Array.from(
            new Set(candidates.map((candidate) => candidate.prefix)),
        );
        ...
        const { buckets, allFromCache } = await loadBuckets(
            userContext.apiBaseUrl || APP_SETTINGS.apiBaseUrl,
            prefixes,
        );
        const matches = matchCandidates(candidates, buckets, allFromCache);
        devDebugLog("urlDecision.lookup.matched", {
            bucketCount: buckets.length,
            bucketEntryCount: buckets.reduce(
        ...
        const action =
            best.entry.decision === "block" && best.entry.confidence < threshold
                ? "allow"
                : best.entry.decision;
        ...
        const result: UrlDecisionResult = {
            ...
            reason:
                action === best.entry.decision
                    ? "matched_hash_suffix"
                    : "matched_below_focus_threshold",
            matchedKey: best.key,
...
export async function lookupCachedUrlDecision(
    rawUrl: string,
    userContext: UserContext = {},
): Promise<UrlDecisionResult> {
    try {
        ...
        if (matches.length === 0) {
            return {
                action: "unknown",
                confidence: 0,
                reason: "no_cached_matching_hash_suffix",
                source: "unknown",
            };
        }
        ...
        return {
            ...
            reason:
                action === best.entry.decision
                    ? "matched_cached_hash_suffix"
                    : "matched_cached_below_focus_threshold",
            matchedKey: best.key,
            source: "cache",
        };
    } catch {
...
async function loadCachedBuckets(
    prefixes: string[],
): Promise<Array<{ prefix: string; entries: LookupBucketEntry[] }>> {
    const buckets: Array<{ prefix: string; entries: LookupBucketEntry[] }> = [];
    for (const prefix of prefixes) {
        const cached = await getCachedBucket(prefix);
        if (cached) {
...
function matchCandidates(
    ...
    for (const candidate of candidates) {
        candidatesByPrefix.set(candidate.prefix, [
            ...(candidatesByPrefix.get(candidate.prefix) || []),
            candidate,
        ]);
    }
    ...
    for (const bucket of buckets) {
        ...
        for (const entry of bucket.entries) {
            const candidate = prefixCandidates.find(
                (item) => item.suffix === entry.suffix,
            );
            if (candidate) {
                matches.push({
                    entry,
...
function compareMatches(left: MatchedEntry, right: MatchedEntry): number {
    ...
    return String(right.entry.updated_at || "").localeCompare(
        String(left.entry.updated_at || ""),
    );
}

function focusModeThreshold(
    focusMode: SiteDecisionFocusMode | undefined,
): number {
    if (focusMode === "normal") {
        return 0.9;
    }
