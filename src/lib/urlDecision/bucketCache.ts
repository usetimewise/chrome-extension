...
export async function getCachedBucket(prefix: string, now = Date.now()): Promise<LookupBucket | null> {
  const cached = await storageGet(cacheKey(prefix)) as Partial<CachedBucket> | null | undefined;
...
export async function saveBuckets(buckets: LookupBucket[], expiresAt: string): Promise<void> {
  ...
  devDebugLog("urlDecision.bucketCache.save", {
    ...
    entryCount: buckets.reduce((total, bucket) => total + bucket.entries.length, 0)
  });
}
