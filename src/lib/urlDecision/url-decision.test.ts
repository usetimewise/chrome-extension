...
import {
    buildLookupKeyCandidates,
    buildLookupKeys,
    parentDomainCandidates,
} from "./lookupKeys.js";
...
test("normalizes URLs without query params, fragments, or private segments", () => {
    assert.deepEqual(
        normalizeUrl("https://www.youtube.com/watch?v=abc#comments"),
        {
            domain: "youtube.com",
            pathSegments: ["watch"],
        },
    ...
    assert.deepEqual(
        normalizeUrl("https://youtube.com/shorts/abc123?feature=share"),
        {
            domain: "youtube.com",
    ...
    assert.deepEqual(
        normalizeUrl(
            "https://example.com/reset-password/abcdefghijklmnopqrstuvwxyzabcdef123456",
        ),
        {
            domain: "example.com",
...
test("builds lookup keys and parent-domain candidates", () => {
    assert.deepEqual(
        buildLookupKeys({ domain: "youtube.com", pathSegments: [] }),
        ["d:youtube.com"],
    );
    assert.deepEqual(
        buildLookupKeys({ domain: "youtube.com", pathSegments: ["shorts"] }),
        ["d:youtube.com", "p1:youtube.com/shorts"],
    );
    assert.deepEqual(
        buildLookupKeys({
            domain: "reddit.com",
            pathSegments: ["r", "programming"],
        }),
        ["d:reddit.com", "p1:reddit.com/r", "p2:reddit.com/r/programming"],
    );
    assert.deepEqual(parentDomainCandidates("mail.google.com"), [
        "mail.google.com",
        "google.com",
    ]);
    assert.deepEqual(parentDomainCandidates("news.bbc.co.uk"), [
        "news.bbc.co.uk",
        "bbc.co.uk",
        "co.uk",
    ]);
...
test("bucket cache returns fresh buckets and drops expired buckets", async () => {
    await saveBuckets(
        [{ prefix: "abcde", entries: [] }],
        new Date(Date.now() + 60_000).toISOString(),
    );
    assert.deepEqual(await getCachedBucket("abcde"), {
        prefix: "abcde",
        entries: [],
    });

    await saveBuckets(
        [{ prefix: "fffff", entries: [] }],
        new Date(Date.now() - 60_000).toISOString(),
    );
...
test("lookupUrlDecision matches suffixes locally and chooses higher specificity", async () => {
    ...
    const result = await lookupUrlDecision(
...
test("lookupUrlDecision returns unknown for no matches and errors", async () => {
        ...
        return {
            ...
            async json() {
                return {
                    ...
                    buckets: body.prefixes.map((prefix) => ({
                        prefix,
                        entries: null as unknown as LookupBucketEntry[],
                    })),
                };
            },
        } as Response;
    ...
    assert.equal(
        (
            await lookupUrlDecision("https://example.com/path", {
                apiBaseUrl: "https://api.example.com",
            })
        ).action,
        "unknown",
    ...
    assert.deepEqual(
        await lookupUrlDecision("https://uncached.example.com/path", {
            apiBaseUrl: "https://api.example.com",
        }),
        {
            action: "unknown",
...
test("lookup candidates include specific keys for parent domains", () => {
    ...
    assert.ok(
        buildLookupKeyCandidates(normalized).includes("p1:youtube.com/shorts"),
    );
});
